                ...
                {item.subitems &&
                  ...
                  item.subitems.map((si: Item, index) => (
                    <div key={index} className="flex justify-start gap-3 mt-5" onClick={() => openNewTab(si.url)}>
                      ...
                      <span className="flex-1">{si.title}
                      
                        <div className="mt-2 flex items-center justify-start gap-1 flex-wrap">
                            {
                              si && si.labels &&
                              si.labels.length > 0 && si.labels.map((l, index) => (
                                <span key={index} className="text-sm p-2 rounded-md border-primary dark:border-blue-700 border-2">
